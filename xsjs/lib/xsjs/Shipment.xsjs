...
case ($.net.http.POST):
	...
	try {
		...
		var resultSet = conn.executeUpdate(query, payload.soId, payload.vehicleId, payload.material, utils.getDate(payload.arrivalTimeAtOrigin),
			utils.getDate(payload.departureAtOrigin), utils.getDate(payload.arrivalTimeAtDestination), utils.getDate(payload.departureAtDestination), utils.getDate(payload.startLoading), utils.getDate(payload.endLoading),
			utils.getDate(payload.startUnloading), utils.getDate(payload.endUnloading), utils.getDate(payload.contactPhoneAtOrigin), payload.contactPersonAtOrigin, payload.contactPhoneAtDestination,
